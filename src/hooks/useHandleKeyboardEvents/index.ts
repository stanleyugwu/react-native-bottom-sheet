 ...
 * @param {RefObject<ComponentRef<typeof Animated.View> | null>} contentWrapperRef Reference to the content wrapper view
      ...
      keyboardShowSubscription.current = Keyboard.addListener(
        ...
        ({ endCoordinates: { height: keyboardHeight } }) => {
          if (sheetOpen) {
            ...
            contentWrapperRef.current?.measure?.(
              (
                _x: number,
                _y: number,
                _width: number,
                ...
                const sheetYOffset = pageY; // Y offset of the sheet after keyboard is out
                ...
                const sheetIsOverlayed =
                  actualSheetHeight - sheetHeight <
                  keyboardAutoCorrectViewHeight;
